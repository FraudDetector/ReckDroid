...
class Handlers
{
	// Set your traffic directory
    static var trafficdir = "F:/Project/ReckDroid/ExploreRecket/output/traffic/";
    // current_pkg.txt contains package name of the app being tested
    static var pkgpath = "F:/Project/ReckDroid/ExploreRecket/output/traffic/current_pkg.txt";
    ...
    */
    ...
    static function OnBeforeRequest(oSession: Session) {
        // "x-OverrideGateway" is for proxy setting
        // oSession["x-OverrideGateway"] = "127.0.0.1:1080";
		...
		// Filter out some irrelevant requests
        if ((!oSession.url.EndsWith(":443")) && (!oSession.hostname.Contains("fiddler")) && (!oSession.hostname.Contains("xiaomi")) && (!oSession.hostname.Contains("miui")) && (!oSession.hostname.Contains("getui")) && (!oSession.url.EndsWith("/generate_204")))
        {
            oSession.utilDecodeRequest();
			
            var pkgobj=new ActiveXObject("Scripting.FileSystemObject");   
            
            //FiddlerObject.alert(oSession.m_clientIP.Split(":"))
			
            if (pkgobj.FileExists(pkgpath))
            {                                           				
                var openpkg = pkgobj.OpenTextFile(pkgpath);
                var pkg = openpkg.ReadAll();
                openpkg.Close();
                var dir0 = trafficdir + pkg + "/";
                
                var fso= new ActiveXObject("Scripting.FileSystemObject");
                if (!fso.FolderExists(dir0)){fso.CreateFolder (dir0);} 		
                
                var sid = oSession.id;
                var dir1 = dir0 + sid + "/";
         
                if (!fso.FolderExists(dir1)){fso.CreateFolder (dir1);} 			
                oSession.SaveSession(dir1 + "Session" +".txt",true);
                //oSession.SaveRequestBody(dir1 + "RequestBody" +".txt");
                              
                try{
                    var urlobj = new ActiveXObject("Scripting.FileSystemObject");
                    var file1 = urlobj.OpenTextFile(dir0 + "urls.txt", 8, true, -2);
                    file1.WriteLine(oSession.fullUrl);    
                    file1.Close();
                }catch(err){
                    //FiddlerObject.alert(err);
                }
                
                try{
                    var hostobj = new ActiveXObject("Scripting.FileSystemObject");
                    var file2 = hostobj.OpenTextFile(dir0 + "hostnames.txt", 8, true, -2);
                    file2.WriteLine(oSession.hostname);
                    file2.Close();
                }catch(err){
                
                } 
                
                //oSession.SaveRequest(dir1 + "Request" +".txt",true);
                //oSession.SaveMetadata(dir1 + "Metadata" +".txt");
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        if ((!oSession.url.EndsWith(":443")) && (!oSession.hostname.Contains("fiddler")) && (!oSession.hostname.Contains("xiaomi")) && (!oSession.hostname.Contains("miui")) && (!oSession.hostname.Contains("getui")) && (!oSession.url.EndsWith("/generate_204")))
        {
            ...
            if (pkgobj.FileExists(pkgpath))
            {
                ...
                if (!fso.FolderExists(dir1)){fso.CreateFolder (dir1);}
                oSession.SaveSession(dir1 + "Session" +".txt",true);
                oSession.SaveResponseBody(dir1 + "ResponseBody" + ".txt");
                
                             
                //oSession.SaveResponse(dir2 + "Response" +".txt",true);
                //oSession.SaveMetadata(dir2 + "Metadata" +".txt");
                     
				//Save all redirects
                try{
                    var testobj = new ActiveXObject("Scripting.FileSystemObject");
                    var file = testobj.OpenTextFile(dir0 + "redirects.txt", 8, true, -2); 
                    var code = oSession.responseCode;
                    // hongbao traffic sign
                    if (oSession.hostname == "localhost"){
                        file.WriteLine("RedPacket"); 
                    }else if (code == 300 || code == 301 || code == 302 || code == 303 || code == 307 ){    // record redirects
                        var redirect = oSession.ResponseHeaders["Location"]
                        if (redirect != ''){
                            file.WriteLine(sid + '#' + redirect); 
                        }else{
                            file.WriteLine(oSession.fullUrl);
                        }   
                    }else if (oSession.ResponseHeaders["Content-Type"].Contains("text/html")){    //record landing pages
                        file.WriteLine(sid + '#' + oSession.fullUrl);
                    }
                    file.Close();
                }catch(err){
                    //FiddlerObject.alert(err);
